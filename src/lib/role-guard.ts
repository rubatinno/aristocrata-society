 ...
 *
 * Admins são exceção: podem entrar por qualquer uma das duas abas.
 */
export async function detectRoleMismatch(
  ...
  if (expected === "mentor") {
    const { data: mentee } = await supabase
      ...
      .eq("id", userId)
      .maybeSingle();
    if (mentee) return 'Essa conta é de mentorado. Entre pela aba "Sou mentorado".';
    return null;
  }

  if (profile) return 'Essa conta é de mentor. Entre pela aba "Sou mentor".';
  return null;
}
